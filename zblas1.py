...
def check1(sfac):
    ...
    for incx in xrange(2):
        for np1 in xrange(4):
            ...
            if icase==6:
                # DZNRM2
                stest1(dznrm2(n, cx, incx), strue2[np1], strue2[np1])
            elif icase==7:
                # DZASUM
                stest1(dzasum(n, cx, incx), strue4[np1], strue4[np1])
            elif icase==8:
                # ZSCAL
                zscal(n, ca, cx, incx)
                ctest(LEN, cx, ctrue5[0][np1][incx], ctrue5[0][np1][incx], sfac)
            elif icase==9:
                # ZDSCAL
                zdscal(n, sa, cx, incx)
                ctest(LEN, cx, ctrue6[0][np1][incx], ctrue6[0][np1][incx], sfac)
            elif icase==10:
                # IZAMAX
    ...
    if icase==8:
        ...
        ca = 0j
        mwpct = [0.0j for i in xrange(5)]
        mwpcs = [1.0j for i in xrange(5)]
        zscal(5, ca, cx, incx)
        ...
        sa = 1.0
        mwpct = [cx[i] for i in xrange(5)]
        mwpcs = [cx[i] for i in xrange(5)]
        zdscal(5, sa, cx, incx)
        ctest(5, cx, mwpct, mwpcs, sfac)
...
def check2(sfac):
    ...
    ns = [0, 1, 2, 4]
    cx1 = [
        0.7-0.8j, -0.4-0.7j, -0.1-0.9j, 0.2-0.8j, -0.9-0.4j, 0.1+0.4j, -0.6+0.6j
        ]
    cy1 = [
        0.6-0.6j, -0.9+0.5j, 0.7-0.6j, 0.1-0.5j, -0.1-0.2j, -0.5-0.3j, 0.8-0.7j
        ]
...
def stest(LEN, scomp, strue, ssize, sfac):
    ...
    for i in xrange(LEN):
        ...
        if not PASS:
            print '{:d4}{:d3}{:d5}{:d5}{:d5}{:d3}{:f36.8}{:f36.8}{f12.4}{f12.4}'.format(
...
def itest2(icomp, itrue):
    ...
    if not PASS:
        ID = icomp - itrue
        print '{:d4}{:d3}{:d5}{:d5}{:d5}{:d36}{:d36}{:d12}'.format(icase, n,
